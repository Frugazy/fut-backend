 ...
 * 6. CHEM SPEED/FORMATTING: Preload blacklist, batch range formats, proper % formatting
 */
...
// ========================================
// CHEM STYLES AREA (FIXED COLUMN MAPPING + SPEED + FORMATTING)
// ========================================
...
// NEW: Preload blacklist once for speed (used by chem builder)
function preloadChemBlacklist() {
  try {
    const data = getSheetData(SHEETS.CHEM_BLACKLIST, 1);
    const full = new Set();
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!name) continue;
      const fullBL = (row[10] || '').toString().toUpperCase() === 'Y';
      const hunterSkip = (row[11] || '').toString().toUpperCase() === 'Y';
      const shadowSkip = (row[12] || '').toString().toUpperCase() === 'Y';
      if (fullBL) full.add(name);
      if (hunterSkip) hunter.add(name);
      if (shadowSkip) shadow.add(name);
      flags[name] = {
        fullBlacklist: fullBL ? 'Y' : 'N',
        hunterSkip: hunterSkip ? 'Y' : 'N',
        shadowSkip: shadowSkip ? 'Y' : 'N'
      };
    }
    return { full: full, hunter: hunter, shadow: shadow, flags: flags };
  } catch (e) {
    Logger.log(`Error preloading blacklist: ${e.toString()}`);
    return { full: new Set(), hunter: new Set(), shadow: new Set(), flags: {} };
  }
}
...
function buildChemStylesDashboard() {
  try {
    ...
    // SPEED: clear and header in batch
    ...
    // SPEED: Preload blacklist once
    const bl = preloadChemBlacklist();

    const playerMap = {};

    function parsePercentCell(cell) {
      if (cell === null || cell === undefined || cell === '') return NaN;
      const num = parseFloat(String(cell).replace('%', '').trim());
    ...
    for (let i = 0; i < hunterData.length; i++) {
      try {
        ...
        if (!playerMap[playerName]) {
          const flags = bl.flags[playerName] || { fullBlacklist: 'N', hunterSkip: 'N', shadowSkip: 'N' };
          playerMap[playerName] = {
            playerName: playerName,
            chemStyle: 'Hunter',
            mprPct: mprPct,               // percent number (e.g., 12.34)
            mprDecimal: mprPct / 100,     // sheet stores as decimal for % format
            hunterBuy: targetBuy,
            ...
            currentPriceShadow: '',
            fullBlacklist: flags.fullBlacklist,
            hunterSkip: flags.hunterSkip,
            shadowSkip: flags.shadowSkip
          };
        } else {
          ...
          if (mprPct > playerMap[playerName].mprPct) {
            playerMap[playerName].mprPct = mprPct;
            playerMap[playerName].mprDecimal = mprPct / 100;
          }
        }
      } catch (e) {
    ...
    for (const playerName in playerMap) {
      ...
      const mprDecimal = (typeof player.mprDecimal === 'number' && !isNaN(player.mprDecimal)) ? player.mprDecimal : 0;
      const row = [
        player.playerName,
        player.chemStyle,
        mprDecimal,                // store as decimal; format as % in sheet and web
        player.hunterBuy,
        player.hunterSell,
        player.shadowBuy,
        player.shadowSell,
    ...
    // BATCH FORMATTING: 1 call for % column, 1 for price block
    chemDashboard.getRange(2, 3, dashboardRows.length, 1).setNumberFormat('0.00%');
    chemDashboard.getRange(2, 4, dashboardRows.length, 6).setNumberFormat('#,##0');
            ...
            chemData.forEach(row => {
                ...
                row.forEach((value, index) => {
                    ...
                    if (index === 0) {
                        ...
                        // MPR is stored as decimal in sheet; render as % in web
